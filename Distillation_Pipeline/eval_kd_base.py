...
from config import DISTILLED_OUTPUT_DIR_B, DATASET_NAME, MAX_LENGTH, TEACHER_MODEL
...
def main():
    print(f"Loading Student model from: {DISTILLED_OUTPUT_DIR_B}")
    tokenizer = AutoTokenizer.from_pretrained(DISTILLED_OUTPUT_DIR_B)
    student = AutoModelForCausalLM.from_pretrained(DISTILLED_OUTPUT_DIR_B, torch_dtype=torch.float32, device_map="auto")
